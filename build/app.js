...
function viewHome() {
  ...
  main.appendChild(h('section', { class: 'section' },
    h('div', { class: 'section-head' },
      ...
      h('a', { class: 'link-more', href: '#/quiz' }, '学完几章？来测一测', icon('right'))),
    grid));
...
function searchEntries(q, pool) {
  ...
  rows.forEach(r => {
    ...
    if (toks.every(t => body.indexOf(t) >= 0)) out.push([toks.every(t => head.indexOf(t) >= 0) ? 0 : 1, r[0]]);
  });
  return out.sort((a, b) => a[0] - b[0]).map(x => x[1]);
}
function viewLibrary() {
  const L = S.lib;
  const input = h('input', { type: 'search', class: 'input', placeholder: '搜中文、英文或提示词：推轨、dutch、halation、雨夜…', 'aria-label': '搜索手法', value: L.q, autocomplete: 'off' });
  ...
  function apply() {
    ...
    list.forEach(s => frag.appendChild(tile(s, { list, cat: L.cat === '' })));
  ...
  if (FULL == null) {
    ...
    loadFull().then(() => { status.textContent = ''; if (L.q.trim() && curPath === '/library') apply(); },
...
function viewQuiz() {
  ...
  main.appendChild(h('section', { class: 'page-head' },
    ...
    h('h1', null, '看片测验'),
    h('p', { class: 'lead' }, '看一段片例，从 4 个选项里认出它用的手法。选完立刻看到定义，错题会列出来方便复习。')));
...
function quizStart(wrap) {
  ...
  wrap.appendChild(h('div', { class: 'quiz-start card' },
    ...
    h('button', { class: 'btn primary', type: 'button', onclick: () => {
      const pool = shuffle(quizPool(sel.value).slice());
      Q.round = { scope: sel.value, qs: pool.slice(0, Math.min(10, pool.length)), i: 0, score: 0, wrong: [], answered: null };
      quizQuestion(wrap);
    } }, '开始 10 题', icon('right')),
...
function quizQuestion(wrap) {
  ...
  function answer(s) {
    ...
    R.answered = s;
    if (s === slug) R.score++; else R.wrong.push(slug);
    quizQuestion(wrap);
  }
...
function quizEnd(wrap) {
  ...
  wrap.appendChild(h('div', { class: 'quiz-end card' },
    ...
    h('p', null, R.score === R.qs.length ? '全对！可以换一章，或者去图鉴里挑战更多手法。' : R.wrong.length + ' 个没认出来，点开复习一下：'),
    R.wrong.length ? h('div', { class: 'tile-grid' }, R.wrong.map(s => tile(s, { list: R.wrong, def: true }))) : null,
    h('div', { class: 'row' },
      h('button', { class: 'btn primary', type: 'button', onclick: () => { Q.round = null; quizStart(wrap); } }, '再来一轮'),
...
function renderPalette() {
  ...
  if (q === '') {
    ...
    hits.slice(0, 40).forEach(s => {
      const r = IDXMAP[s];
      palItems.push({ kind: catZh(r[3]), t: r[2], en: r[1], thumb: r[5], go: () => openEntry(s, hits.slice(0, 40)) });
    });
  }
  palSel = 0;
  palList.textContent = '';
  ...
  palItems.forEach((it, i) => {
    const row = h('div', { class: 'pal-item', role: 'option', id: 'pal-' + i, 'aria-selected': i === 0 ? 'true' : 'false',
      ...
      h('span', { class: 'pal-main' }, h('b', null, it.t), it.en ? h('small', { lang: 'en' }, it.en) : it.sub ? h('small', null, it.sub) : null),
      h('span', { class: 'pal-kind' }, it.kind));
    palList.appendChild(row);
  });
  if (FULL == null && q) palList.appendChild(h('p', { class: 'pal-empty' }, '全文索引载入中，稍后会包含提示词里的匹配…'));
}
function select(i) {
