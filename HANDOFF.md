...
data-search.js      生成物：讲解正文中文全文索引（1.3MB / gzip 约 470KB），图鉴或搜索面板输入搜索词时才加载
...
- `#/library` 手法图鉴：424 条缩略图网格，搜索分三档排序：名称/别名 → 定义与提示词 → 只在讲解正文里提到（卡片上显示「讲解中提到」上下文片段并高亮关键词；讲解索引只含中文）、13 分类、视频/静帧、只看收藏。
...
- `#/quiz` 看片测验：按全部 / 单章 / 错题本出 10 题，4 选 1（干扰项优先同分类），数字键 1–4 作答，可随时「结束本轮」。错题本存 localStorage `ct-wrong`（slug → 答错次数、最近时间），答错自动收进、答对一次移出；开始页列出错题卡片，可「只练错题」或清空（二次点击确认，不用 confirm 弹窗）。首页测验入口会提示错题数。
...
### 第六轮（2026-09-24 错题本 + 讲解全文搜索）

- 图鉴搜索：「库布里克」2 条（1 条来自讲解，片段高亮）、「荷兰角」13 条（8 条来自讲解）、「halation」10 条、「希区柯克 眩晕」多词命中；搜索面板「眩晕」依次为课程 → 手法 → 讲解命中。
- 错题本：一轮故意答错 8 题 → `ct-wrong` 8 条、开始页出现「错题本（8 个）」选项与 8 张卡（显示「答错 1 次」）；「只练错题」答对一题 → 反馈「已移出错题本」，剩 7 条。
- 390 视口测验 / 图鉴 / 搜索结果无横向溢出；控制台 0 报错。
...
- 已做（2026-09-24）：错题本持久化；讲解正文纳入全文搜索。
- 仍可考虑：讲解英文原文未进搜索索引（英文检索依赖名称、定义与提示词）。
